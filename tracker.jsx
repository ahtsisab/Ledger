...
export default function PokerTracker() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get("ledger-game-state");
      ...
      try {
        const hResult = await window.storage.get("ledger-game-history");
        if (hResult && hResult.value) {
          setHistory(JSON.parse(hResult.value));
        }
      } catch (e) {}
      setLoaded(true);
    })();
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        await window.storage.set("ledger-game-state", JSON.stringify({
          phase,
          players,
          settlements,
          cashoutAmounts,
  ...
  const finishGame = async () => {
    ...
    try { await window.storage.set("ledger-game-history", JSON.stringify(newHistory)); } catch (e) {}
  };
  ...
  const resetGame = async () => {
    ...
    try { await window.storage.delete("ledger-game-state"); } catch (e) {}
  };

  const deleteHistoryGame = async (gameId) => {
  ...
  const clearAllHistory = async () => {
    ...
    try { await window.storage.delete("ledger-game-history"); } catch (e) {}
